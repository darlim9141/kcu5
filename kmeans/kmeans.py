...
# Pillow, tensorflow, VGG16 등은 더 이상 필요 없습니다!
from sklearn.cluster import KMeans
from sklearn.decomposition import PCA
from sklearn.manifold import TSNE

# --- 1 & 2 단계가 사라졌습니다! ---
# load_and_preprocess_images 함수와 extract_features 함수는 이제 필요 없으므로 삭제합니다.

# --- 3. 시각화 함수 (이전과 동일) ---
def visualize_clusters(tsne_results, labels, title):
    """t-SNE로 변환된 결과를 색상별로 구분하여 시각화합니다."""
...
if __name__ == "__main__":
    ...
    # 2. VGG16 모델 준비 및 특징 추출 단계는 완전히 건너뜁니다.
    # 이미 특징 추출이 완료된 데이터를 사용하기 때문입니다.

    # 3. PCA로 차원 축소 (이전과 동일)
    print("\n3. PCA를 사용하여 특징 벡터의 차원을 축소합니다...")
    # K-means의 성능과 안정성을 위해 주성분 50개로 차원을 축소합니다.
    pca = PCA(n_components=50, random_state=42)
    # 바로 features 변수를 사용합니다.
    features_pca = pca.fit_transform(features)
    print(f"PCA로 축소된 특징 벡터의 형태: {features_pca.shape}")

    # 4. K-means 클러스터링 실행 (이전과 동일)
    print("\n4. K-means 클러스터링을 진행합니다...")
    kmeans = KMeans(n_clusters=4, random_state=42, n_init=10)
    cluster_labels = kmeans.fit_predict(features_pca)

    # 5. t-SNE로 시각화를 위한 2차원 축소 (이전과 동일)
